...
class MongoStorage(mongo: MongoClient) extends Storage[WriteResult] {
  ...
  def findProducts(ids: Seq[Tuple2[String, String]], country: String, fields: Seq[String]) : Future[Iterable[Product]] = {
    Future {
      ...
      val query = new StringBuilder(128).append("{ $or: [")
  ...
  def findProductByIds(ids: Array[String], country: String, fields: Seq[String]) : Future[Iterable[Product]] = {
    Future {
      val productCollection = jongo.getCollection("products")
      val query = new StringBuilder(128).append("{ skus.countries.code:#, $or: [")
      ids.foreach(t => query.append("{_id:#},"))
      query.setLength(query.length - 1)
      query.append("]}")
      ...
      filterSkus(country, productCollection.find(query.toString(), parameters:_*).projection(projectionProduct(fields)).as(classOf[Product]))
    }
  }
